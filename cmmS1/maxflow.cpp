...
// 将节点 i 设置为活动状态，如果它尚未在活动列表中，则将其添加到列表中
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::set_active(node *i)
{
	if (!i->next) // 首先检查节点 i 是否已经在列表中
	{
		/* it's not in the list yet */
		if (queue_last[1]) queue_last[1] -> next = i; // 如果 i 不在列表中（i->next 为 NULL），则将其添加到第二个队列的末尾
		else               queue_first[1]        = i; // 如果第二个队列之前为空，则 i 也成为该队列的起始节点
		queue_last[1] = i; // 指向它自己，这表示它在列表中，但目前没有后续节点
		i -> next = i;
	}
}
...
/***********************************************************************/
// 将节点 i 标记为孤立节点（没有父节点），并将其添加到孤立节点列表的前端
template <typename captype, typename tcaptype, typename flowtype> 
...
// 将节点 i 标记为孤立节点（没有父节点），并将其添加到孤立节点列表的后端
template <typename captype, typename tcaptype, typename flowtype> 
...
/***********************************************************************/
// 将节点 i 添加到变更列表中，这个列表用于在重用树的初始化过程中跟踪哪些节点需要更新
template <typename captype, typename tcaptype, typename flowtype> 
...
// 通过增广路径来增加流量。它找到增广路径的瓶颈容量，并更新沿该路径的边的容量
template <typename captype, typename tcaptype, typename flowtype> 
...
/***********************************************************************/
// 用于源点孤立节点，尝试为孤立节点找到新的父节点，或者将它们添加到变更列表中
template <typename captype, typename tcaptype, typename flowtype> 
...
// 用于汇点孤立节点，尝试为孤立节点找到新的父节点，或者将它们添加到变更列表中
template <typename captype, typename tcaptype, typename flowtype> 
...
/***********************************************************************/
// 主函数
// 接受一个布尔参数 reuse_trees，指示是否重用搜索树
// 接受一个指向变更列表的指针
// 函数执行最大流算法，并返回从源点到汇点的最大流量
template <typename captype, typename tcaptype, typename flowtype> 
	flowtype Graph<captype,tcaptype,flowtype>::maxflow(bool reuse_trees, Block<node_id>* _changed_list)
{
	...
	if (maxflow_iteration == 0 && reuse_trees) { if (error_function) (*error_function)(const_cast<char*>("reuse_trees cannot be used in the first call to maxflow()!")); exit(1); }
	if (changed_list && !reuse_trees) { if (error_function) (*error_function)(const_cast<char*>("changed_list cannot be used without reuse_trees!")); exit(1); }
