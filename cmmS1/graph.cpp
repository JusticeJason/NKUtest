...
// captype（容量类型），tcaptype（残余容量类型），flowtype（流量类型）
	  ...
	  error_function(err_function)
{
	...
	// 如果内存分配失败，调用错误处理函数并退出程序
	if (!nodes || !arcs) { if (error_function) (*error_function)(const_cast<char*>("Not enough memory!")); exit(1); }
...
template <typename captype, typename tcaptype, typename flowtype> 
	Graph<captype,tcaptype,flowtype>::~Graph() // 析构函数
{
	if (nodeptr_block) 
	{ 
...
// 重置图的状态，释放nodeptr_block，重置节点和边的计数器
template <typename captype, typename tcaptype, typename flowtype> 
	...
	void Graph<captype,tcaptype,flowtype>::reallocate_nodes(int num)
{
	...
	if (!nodes) { if (error_function) (*error_function)(const_cast<char*>("Not enough memory!")); exit(1); }
	...
	void Graph<captype,tcaptype,flowtype>::reallocate_arcs()
{
	...
	if (!arcs) { if (error_function) (*error_function)(const_cast<char*>("Not enough memory!")); exit(1); }
